...
import { resolveSiteIcon, useSiteContent } from "@/lib/siteContent";
import React from "react";

const Process = () => {
  const { ourProcessData } = useSiteContent();
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
          {ourProcessData.list.map((feature, index) => {
            const Icon = resolveSiteIcon(feature.icon);
            return (
              <div
                key={index}
              ...
              >
                ...
                <div className="flex items-center justify-between gap-3 relative z-[1]">
                  <div className="rounded-xl bg-primary text-white p-3 shadow-md shadow-primary/20 group-hover:scale-105 transition-transform">
                    <Icon size={22} />
                  </div>
                </div>

...
type EstimateImagesFieldProps = {
  images: string[];
  onChange: (images: string[]) => void;
};

export default function EstimateImagesField({
  images,
  onChange,
}: EstimateImagesFieldProps) {
  ...
  return (
    <div className="space-y-3 md:col-span-2">
      <div className="flex items-center justify-between gap-3">
        <div>
          <Label>Estimate images</Label>
          <p className="mt-1 text-xs text-muted-foreground">
            Optional. Upload photos to show on the public estimate (before the
            video). Paths are stored without the media server base URL.
          </p>
        </div>
        <Button
          type="button"
